  ...
  methods: {
      ...
      const itemList = [
        {
          name: '安全巡检',
          handler: () => {
            this.$utils.toUrl('/pages/safe/add2?planNature=4')
          }
        },
        {
          name: '安全定点巡视',
          handler: () => {
            this.$utils.toUrl('/pages/safe/add2?planType=3&planNature=5')
          }
        }
      ]
      uni.showActionSheet({
    ...
    handleInspectItemClick(row) {
      if (row.rectifyStatus === 0) {
        //未检查
        this.$utils.toUrl('/pages/safe/add2?id=' + row.inspectTaskId)
      } else {
