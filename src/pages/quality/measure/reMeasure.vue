  ...
  <div class="main">
    <scroll-view class="scroll-roll" scroll-y>
      ...
      <uni-card v-for="item in activePoint.measurePositionList || []" :key="item.id">
        ...
        <uni-form-item label="设计值" :isRequired="!item.standardValue">
          <input
        ...
        <uni-form-item label="测量值" :isRequired="!item.standardValue">
          <input
        ...
        <uni-form-item label="是否合格" isRequired>
          <div v-if="!item.standardValue">{{ ['不合格', '合格'][item.status] }}</div>
          <ss-switch v-else v-model="item.status" onText="合格" offText="不合格" :onVal="1" :offVal="0"></ss-switch>
        </uni-form-item>
      </uni-card>
  ...
  methods: {
    ...
    submit() {
      ...
      for (let point of measurePositionList) {
        if ((point.measureValue === null || point.designValue === null) && !point.standardValue) {
    ...
    calculateItem(item) {
      if (!item.standardValue && _.isNumber(item.designValue) && _.isNumber(item.measureValue)) {
