        ...
        <scroll-view class="scroll-roll" scroll-y>
          <div class="list">
            ...
            <div
              ...
              :key="item.id"
              class="item"
              :class="{ active: item.id === a.id, backGroundColor: item.findChildrenBind === false }"
              @click="a = item"
            >
              {{ item.label }}
            </div>
          ...
          <div v-if="a.children" class="list">
            ...
            <div
              ...
              :key="item.id"
              class="item"
              :class="{ active: item.id === b.id, backGroundColor: item.findChildrenBind === false }"
              @click="b = item"
            >
              {{ item.label }}
            </div>
          ...
          <div v-if="b.children" class="list">
            ...
            <div
              ...
              :key="item.id"
              class="item"
              :class="{ active: item.id === c.id, backGroundColor: item.findChildrenBind === false }"
              @click="c = item"
            >
              {{ item.label }}
            </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      if (!this.c.drawingUrl) {
        this.$utils.showToast('该户型没有图纸，无法测量')
        return false
      }
      this.form.measureLocation = _.map(_.pick(this, ['a', 'b', 'c']), 'label').join('-')
      this.form.measureDrawingId = this.c.drawingId
      this.form.measureDrawingUrl = this.c.drawingUrl
      this.form.measureLocationId = this.c.id
      this.close()
    },
    remove() {},
    open() {
