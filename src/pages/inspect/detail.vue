<template>
  <div class="main">
    ...
    <uni-card title="检查信息">
      ...
      <uni-form-item :label="form.beInspectedType === '1' ? '受检班组' : '受检部门'">
    ...
    <uni-card title="检查结果">
      ...
      <!--<template v-if="form.inspectStatus === 2">
        &lt;!&ndash; <uni-form-item label="不合格位置">
          1号楼，3好，5层 2号楼
        </uni-form-item>
      ...
      <div v-if="form.inspectStatus === 1" :key="1">
        <uni-form-item label="备注">
          <view>{{ form.inspectTaskResultList[0].taskRemark }}</view>
        </uni-form-item>
        <uni-form-item class="file-item" label="现场照片">
          <uni-attachment v-model="form.inspectTaskResultList[0].taskFileId"></uni-attachment>
        </uni-form-item>
      </div>
    ...
    <div v-if="form.inspectStatus === 2" :key="2">
      <uni-card v-for="(item, index) in form.inspectTaskResultList" title="问题明细" :key="index">
        <uni-form-item label="检查部位">
          <view>{{ item.inspectPosition }}</view>
        </uni-form-item>
        <uni-form-item label="问题描述">
          <view>{{ item.issueDescription }}</view>
        </uni-form-item>
        <uni-form-item label="问题等级">
          <view>{{ $getLabel(issueLevelList, item.issueLevel) }}</view>
        </uni-form-item>
        <uni-form-item label="紧急程度">
          <view>{{ $getLabel(emergencyLevelList, item.emergencyLevel) }}</view>
        </uni-form-item>
        <uni-form-item label="备注">
          <view>{{ item.taskRemark }}</view>
        </uni-form-item>
        <uni-form-item class="file-item" label="现场照片">
          <uni-attachment v-model="item.taskFileId"></uni-attachment>
        </uni-form-item>
      </uni-card>
    </div>
