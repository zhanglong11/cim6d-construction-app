<template>
  <view class="ss-switch-box" @click="switchChange">
    ...
    <view
      ...
      class="ss-switch"
      :style="[style_]"
      :class="{ 'ss-switch__active': switchSel_, 'ss-switch__disabled': disabled }"
    >
      ...
      <view v-if="loading && loadingMask_" class="ss-switch__mask" :style="{ backgroundColor: maskBg }"></view>
      <view v-if="loading" class="ss-switch__loading spin" :style="{ color: loadingColor }"></view>
    </view>
  ...
  mounted() {
    this.$nextTick(() => {
      setTimeout(() => {
        ...
        query
          .selectAll('.ss-switch__text, .ss-switch__slide, .ss-switch')
          .boundingClientRect(data => {
            let switchWidth = 0
            let textData = data
              .filter(o => o.dataset.name === 'text')
              .map(o => {
                let width = o.width
                switchWidth += width
                return width
  ...
  methods: {
    ...
    switchChange() {
      ...
      if (typeof this.value === 'object') {
        ...
        if (this.switchSel_) {
          value.splice(
            value.findIndex(o => o === this.onVal),
            1
          )
        } else {
          value.push(res)
...
.ss-switch {
  ...
  .ss-switch__loading {
    ...
    box-shadow: 14rpx -10rpx 0 0.5px currentColor, /* 右 */ 0 16rpx currentColor,
      /* 下 */ -14rpx -10rpx 0 0.5px currentColor, /* 左 */ 0 -16rpx 0 1px currentColor,
      /* 右上, 1.5px扩展 */ 14rpx 10rpx currentColor, /* 右下 */ -14rpx 10rpx currentColor; /* 左下 */
  }
